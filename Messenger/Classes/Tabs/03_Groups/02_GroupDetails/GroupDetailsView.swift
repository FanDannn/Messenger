...
class GroupDetailsView: UIViewController {
	...
	func loadChats() {
		...
		chat = realm.object(ofType: Chat.self, forPrimaryKey: chatId)

		labelName.text = chat.groupName

		if let person = realm.object(ofType: Person.self, forPrimaryKey: chat.groupOwnerId) {
			labelFooter1.text = "Created by \(person.fullname)"
			labelFooter2.text = Convert.timestampToMediumTime(chat.createdAt)
		}
	}
	...
	func actionMoreOwner() {
		...
		alert.addAction(UIAlertAction(title: "Delete Group", style: .destructive, handler: { action in
			self.chat.update(groupDeleted: true)
	...
	func actionRenameGroup() {
		...
		alert.addTextField(configurationHandler: { textField in
			textField.text = self.chat.groupName
		...
		alert.addAction(UIAlertAction(title: "Save", style: .default, handler: { action in
			...
			if let text = textField.text {
				if (text.count != 0) {
					self.chat.update(groupName: text)
