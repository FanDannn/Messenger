...
class Details: NSObject {
	...
	// MARK: -
	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func deleteIds() -> [String] {

		let predicate = NSPredicate(format: "userId == %@ AND isDeleted == YES", AuthUser.userId())
		let details = realm.objects(Detail.self).filter(predicate)

		var chatIds: [String] = []
		for detail in details {
			chatIds.append(detail.chatId)
		}
		return chatIds
	}

	//---------------------------------------------------------------------------------------------------------------------------------------------
	class func archiveIds() -> [String] {

		let predicate = NSPredicate(format: "userId == %@ AND isArchived == YES", AuthUser.userId())
		let details = realm.objects(Detail.self).filter(predicate)
